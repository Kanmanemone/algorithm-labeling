...
## 구현·기본기 (Implementation & Fundamentals)
...
### 파싱/수식 처리 (Parsing / Expression Parsing)
...
### 자료구조 일반 (Data Structures (General))
...
### 모노토닉 스택/큐 (Monotonic Stack / Queue)
...
### 인덱스 트리(BIT) (Binary Indexed Tree)
...
### 누적합과 좌표 압축 (Prefix Sum & Coordinate Compression)
...
**대표 예시**: "Count of Range Sum" — LeetCode — https://leetcode.com/problems/count-of-range-sum/ (모든 구간 쿼리를 한꺼번에 놓고 병합 정렬/BIT로 순서를 재배치해 처리하는 오프라인 접근의 대표적인 예)
...
### 그래프 이론 일반 (Graph Theory (General))
...
### 이분 그래프와 매칭 (Bipartite Graph & Matching)
...
### 소수 판정과 소인수분해 (Primality Test & Prime Factorization)
...
## 조합론과 확률 (Combinatorics & Probability)
...
### KMP 문자열 매칭 (KMP String Matching)
...
### 다각형과 선분 기하 (Polygon & Line Geometry)
...
## 구성적 알고리즘 (Constructive Algorithms)
...
| 원본 태그 | 병합 라벨 |
|---|---|
| implementation, ad_hoc, arithmetic, case_work, precomputation, traceback | 구현 (Implementation) |
| simulation | 시뮬레이션 (Simulation) |
| parsing | 파싱/수식 처리 (Parsing / Expression Parsing) |
| recursion | 재귀 (Recursion) |
| backtracking | 백트래킹 (Backtracking) |
| bruteforcing | 완전탐색 (Brute Force) |
| sorting | 정렬 (Sorting) |
| binary_search, parametric_search | 이분 탐색 (Binary Search) |
| ternary_search | 삼분 탐색 (Ternary Search) |
| two_pointer | 투 포인터 (Two Pointers) |
| sliding_window | 슬라이딩 윈도우 (Sliding Window) |
| data_structures | 자료구조 일반 (Data Structures (General)) |
| stack, queue, deque | 스택/큐/덱 (Stack / Queue / Deque) |
| priority_queue | 우선순위 큐/힙 (Priority Queue / Heap) |
| linked_list | 연결 리스트 (Linked List) |
| set, hash_set, tree_set | 집합과 맵 (Set & Map) |
| hashing | 해싱 (Hashing) |
| trie | 트라이 (Trie) |
| segtree, lazyprop | 세그먼트 트리 (Segment Tree) |
| disjoint_set | 유니온 파인드 (Union-Find) |
| prefix_sum, difference_array, coordinate_compression | 누적합과 좌표 압축 (Prefix Sum & Coordinate Compression) |
| offline_queries | 오프라인 쿼리 (Offline Queries) |
| graphs, graph_traversal | 그래프 이론 일반 (Graph Theory (General)) |
| bfs, 0_1_bfs, grid_graph | BFS (Breadth-First Search) |
| dfs, grid_graph | DFS (Depth-First Search) |
| dijkstra, shortest_path, floyd_warshall, bellman_ford | 최단 경로 (Shortest Path) |
| mst | 최소 스패닝 트리 (Minimum Spanning Tree) |
| topological_sorting, dag | 위상 정렬 (Topological Sort) |
| bipartite_graph, bipartite_matching | 이분 그래프와 매칭 (Bipartite Graph & Matching) |
| flow | 네트워크 유량 (Network Flow) |
| scc | 강한 연결 요소 (Strongly Connected Component) |
| trees | 트리 기초 (Tree Basics) |
| dp_tree | 트리에서의 DP (Tree DP) |
| lca | 최소 공통 조상 (Lowest Common Ancestor) |
| tree_diameter | 트리의 지름 (Tree Diameter) |
| dp | DP 기초 (Dynamic Programming Basics) |
| knapsack | 배낭 문제 (Knapsack) |
| lis | 최장 증가 부분 수열 (Longest Increasing Subsequence) |
| lcs | 최장 공통 부분 수열 (Longest Common Subsequence) |
| dp_bitfield | 비트마스킹 DP (Bitmask DP) |
| maximum_subarray | 최대 부분 배열 (Maximum Subarray) |
| greedy | 그리디 (Greedy) |
| divide_and_conquer | 분할 정복 (Divide and Conquer) |
| math, parity | 수학 일반 (Math (General)) |
| number_theory | 정수론 (Number Theory) |
| euclidean | 최대공약수와 최소공배수 (GCD & LCM) |
| prime_factorization, sieve, primality_test | 소수 판정과 소인수분해 (Primality Test & Prime Factorization) |
| modular_multiplicative_inverse, exponentiation_by_squaring | 모듈러 연산 (Modular Arithmetic) |
| combinatorics | 조합론 (Combinatorics) |
| probability | 확률과 통계 (Probability & Statistics) |
| string | 문자열 (String) |
| kmp | KMP 문자열 매칭 (KMP String Matching) |
| geometry | 기하 (Geometry) |
| convex_hull | 볼록 껍질 (Convex Hull) |
| polygon_area, line_intersection | 다각형과 선분 기하 (Polygon & Line Geometry) |
| bitmask | 비트마스킹 (Bit Manipulation) |
| game_theory | 게임 이론 (Game Theory) |
| constructive | 구성적 알고리즘 (Constructive Algorithms) |
...
| 원본 태그 | 병합 라벨 |
|---|---|
| Array, Matrix | 구현 (Implementation) |
| Simulation | 시뮬레이션 (Simulation) |
| Recursion | 재귀 (Recursion) |
| Backtracking | 백트래킹 (Backtracking) |
| Enumeration | 완전탐색 (Brute Force) |
| Sorting | 정렬 (Sorting) |
| Binary Search | 이분 탐색 (Binary Search) |
| Two Pointers | 투 포인터 (Two Pointers) |
| Sliding Window | 슬라이딩 윈도우 (Sliding Window) |
| Graph Theory | 그래프 이론 일반 (Graph Theory (General)) |
| Stack, Queue | 스택/큐/덱 (Stack / Queue / Deque) |
| Monotonic Stack, Monotonic Queue | 모노토닉 스택/큐 (Monotonic Stack / Queue) |
| Heap (Priority Queue) | 우선순위 큐/힙 (Priority Queue / Heap) |
| Linked List | 연결 리스트 (Linked List) |
| Hash Table | 집합과 맵 (Set & Map) |
| Trie | 트라이 (Trie) |
| Segment Tree | 세그먼트 트리 (Segment Tree) |
| Binary Indexed Tree | 인덱스 트리(BIT) (Binary Indexed Tree) |
| Union-Find | 유니온 파인드 (Union-Find) |
| Prefix Sum | 누적합과 좌표 압축 (Prefix Sum & Coordinate Compression) |
| Depth-First Search | DFS (Depth-First Search) |
| Breadth-First Search | BFS (Breadth-First Search) |
| Shortest Path | 최단 경로 (Shortest Path) |
| Minimum Spanning Tree | 최소 스패닝 트리 (Minimum Spanning Tree) |
| Topological Sort | 위상 정렬 (Topological Sort) |
| Strongly Connected Component | 강한 연결 요소 (Strongly Connected Component) |
| Tree, Binary Tree, Binary Search Tree | 트리 기초 (Tree Basics) |
| Dynamic Programming, Memoization | DP 기초 (Dynamic Programming Basics) |
| Divide and Conquer | 분할 정복 (Divide and Conquer) |
| Math | 수학 일반 (Math (General)) |
| Number Theory | 정수론 (Number Theory) |
| Combinatorics, Counting | 조합론 (Combinatorics) |
| Probability and Statistics | 확률과 통계 (Probability & Statistics) |
| String | 문자열 (String) |
| Geometry | 기하 (Geometry) |
| Bit Manipulation, Bitmask | 비트마스킹 (Bit Manipulation) |
| Game Theory | 게임 이론 (Game Theory) |
| Design | 자료구조 설계 (Design) |
| Greedy | 그리디 (Greedy) |
...
| brute force | 완전탐색 (Brute Force) |
| sortings | 정렬 (Sorting) |
| binary search | 이분 탐색 (Binary Search) |
| ternary search | 삼분 탐색 (Ternary Search) |
| two pointers | 투 포인터 (Two Pointers) |
| data structures | 자료구조 일반 (Data Structures (General)) |
| dsu | 유니온 파인드 (Union-Find) |
| dfs and similar | DFS (Depth-First Search) |
| flows | 네트워크 유량 (Network Flow) |
| trees | 트리 기초 (Tree Basics) |
| dp | DP 기초 (Dynamic Programming Basics) |
| divide and conquer | 분할 정복 (Divide and Conquer) |
| greedy | 그리디 (Greedy) |
...
| number theory | 정수론 (Number Theory) |
| combinatorics | 조합론 (Combinatorics) |
| probabilities | 확률과 통계 (Probability & Statistics) |
| strings | 문자열 (String) |
| expression parsing | 파싱/수식 처리 (Parsing / Expression Parsing) |
| geometry | 기하 (Geometry) |
| bitmasks | 비트마스킹 (Bit Manipulation) |
| games | 게임 이론 (Game Theory) |
| hashing | 해싱 (Hashing) |
| constructive algorithms | 구성적 알고리즘 (Constructive Algorithms) |
| shortest paths | 최단 경로 (Shortest Path) |
...
|---|---|
| 해시 | 집합과 맵 (Set & Map) |
| 스택/큐 | 스택/큐/덱 (Stack / Queue / Deque) |
| 힙 | 우선순위 큐/힙 (Priority Queue / Heap) |
| 정렬 | 정렬 (Sorting) |
| 완전탐색 | 완전탐색 (Brute Force) |
| 탐욕법 | 그리디 (Greedy) |
| 동적계획법 | DP 기초 (Dynamic Programming Basics) |
| 깊이/너비 우선 탐색 | BFS (Breadth-First Search), DFS (Depth-First Search) |
| 이분탐색 | 이분 탐색 (Binary Search) |
| 그래프 | 그래프 이론 일반 (Graph Theory (General)) |
