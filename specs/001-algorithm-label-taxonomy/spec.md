...
- **FR-009**: 라벨 집합의 완성 상태는 조사 시점(이번 기능 수행 시점) 기준으로 네 출처(FR-001)의 공개 태그를 모두 흡수했을 때 완료로 간주하는 정적 스냅샷 산출물로 정의한다. 완료 이후 출처의 태그 체계 변경사항을 지속적으로 반영하는 것은 이번 기능의 범위에 포함하지 않으며, 필요 시 별도의 후속 갱신 작업으로 다룬다.
...
- **SC-001**: 조사 시점(스냅샷 기준) 기준으로 네 출처의 공개 태그 목록에 있는 태그 중 100%가 통합 라벨 집합의 하나 이상의 라벨에 매핑되어 있다 (baseline: 0% — 현재 통합 라벨 집합 없음).
...
- 최종 하위 태그 개수에 대한 사전 목표치는 두지 않으며, 여러 출처의 태그를 병합하는 과정에서 자연스럽게 정해지는 규모를 그대로 받아들인다.
